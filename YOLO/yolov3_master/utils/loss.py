...
from YOLO.yolov3_master.utils.general import bbox_iou
from YOLO.yolov3_master.utils.torch_utils import is_parallel
...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        :param targets: targets.shape = (nt, 6) , 6=icxywh,i表示第一张图片，c为类别，然后为坐标xywh
        :param model: 模型
        Returns:
         """
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                所以中心点回归的公式变为：
                xy.sigmoid() * 2. - 0.5 + cx
                每个标签框采用了更多的anchor来回归，笔者亲测这个操作是增加了召回率，
                但准确率有所下降总体来说map会有所增加
                """
                j, k = ((gxy % 1. < g) & (gxy > 1.)).T
                l, m = ((gxi % 1. < g) & (gxi > 1.)).T
